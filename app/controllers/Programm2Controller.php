<?php
session_start();
class Programm2Controller extends Controller
{
public function action_index(){
    $col=20;
    $v=new View("programm2");
    unset($_SESSION['find']);
    $v->title="Електронний реєстр хворих сахарним діабетом";
    $c=new Programm2Controller();

        $limit=0;
        $offset=$limit+$col;
        $v->spis=MainModel::Instance()->getKartaPlimit($limit,$col);
        $v->all=MainModel::Instance()->getKartaCountlimit();
...
//echo "7777";die();
    ...
    public function action_findSkin(){
        $col=20;
        unset($_SESSION['find']);
        $v=new View("programm2");
        $v->title="Електронний реєстр хворих сахарним діабетом";
