<?php
session_start();
class NewvizitController extends Controller
{
    public function action_index(){
        ...
        if(isset($_POST['id_k'])){
        //    $id=PacientModel::Instance()->newvizit($_POST['id_k'],$_POST['nomPac']);
            $v->maxv=PacientModel::Instance()->getmaxv($_POST['nomPac']);
            //echo $id;die();
           ...
           // $v->hist=MainModel::Instance()->gethist($id);
            $v->vizit=MainModel::Instance()->getVizit1($v->karta[0]['nomPac']);

            $v->post=$v->karta;
        }
        $c=new NewvizitController();
