<?php
session_start();
class Ajax9Controller extends Controller
{
...
?>
    ...
    border-color: #d9d9d9;"><th>№карт</th><th>ПІБ</th><th>Стать</th><th>Рік нар.</th><th>Тип діабету</th><th>Населений пункт</th>
            ...
            foreach($spis as $item):
                ...
                echo "id='o{$item['id']}' onclick='save2(this.id)'><td><form method='post' name='k{$item['id']}' id='k{$item['id']}' action=''><input type='hidden' name='id_k' value='{$item['id']}'></form> {$item['id']}</td>
