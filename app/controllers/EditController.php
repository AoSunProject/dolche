...
if(isset($_POST['id_k'])){
    ...
    $v->karta=MainModel::Instance()->getkarta2($_POST['id_k']);
    //$v->karta=MainModel::Instance()->getkarta1($_POST['id_k']);

    $v->hist=MainModel::Instance()->gethist($_POST['id_k']);
    $v->vizit=MainModel::Instance()->getVizit($_POST['id_k']);

    $v->post=$_POST;
}
elseif(isset($_GET['id_k'])){

    $v->karta=MainModel::Instance()->getkarta20($_GET['id_k']);
    //$v->karta=MainModel::Instance()->getkarta1($_POST['id_k']);

    $v->hist=MainModel::Instance()->gethist($_GET['id_k']);
    $v->vizit=MainModel::Instance()->getVizit($_GET['id_k']);
...
$arr=$_POST;
       ...
       PacientModel::Instance()->save($this->post("id"),
          ...
          $periodGolodS, $this->apostr($this->post("death1")),
           ...
           $this->post("statysA"),
           $this->post("vlasnik"),
           $this->post("pomer"),$this->post("nomPac"),$this->post("datedeth"),$this->post("sincdeth")
...
$_SESSION['find']=$_POST['post1'];
        ...
        $v->title="Електронний реєстр хворих сахарним діабетом";
        $c=new EditController();
        $v->karta=MainModel::Instance()->getkarta1($this->post("id"));
        $v->hist=MainModel::Instance()->gethist($this->post("id"));
        $v->vizit=MainModel::Instance()->getVizit1($this->post("nomPac"));
        $v->post=$_POST;
        $v->first=1;
        $v->viewTemplate();
        $this->responce($v);
        }
    public function action_savevizit(){
        if(isset($_POST['g1o']) and !isset($_POST['g2o']) and !isset($_POST['g3o'])) {
            $periodGolodO=$_POST['g1o'];
        }
        ...
        $arr=$_POST;
        $id=PacientModel::Instance()->addvizit(
            $this->apostr($this->post("name")), $this->apostr($this->post("fname")),
            $this->post("sname"), $this->apostr($this->post("ychas")), $this->post("nomIss"), $this->post("sex")
            , $this->post("datB"), $this->apostr($this->post("city")), $this->apostr($this->post("rajon")),
            $this->apostr($this->post("work")), $this->apostr($this->post("week")),
      ...
      /*  $_SESSION['find']=$_POST['post1'];
        ...
        $v->title="Електронний реєстр хворих сахарним діабетом";
        $c=new EditController();
        $v->karta=MainModel::Instance()->getkarta1($id);
        $v->hist=MainModel::Instance()->gethist($id);
        $v->vizit=MainModel::Instance()->getVizit1($this->post("nomPac"));
        $v->post=$_POST;
        $v->first=1;
        $v->viewTemplate();
        $this->responce($v);*/
        $this->redirect(Url::local("edit?id_k={$id}"));
    }
}